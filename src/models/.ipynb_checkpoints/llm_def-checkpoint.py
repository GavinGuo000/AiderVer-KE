...
# Set proxy for requests (commented out to avoid connection issues)
...
class Qwen3(BaseEngine):
    def __init__(self, model_name_or_path: str):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            ...
            torch_dtype="auto",
            device_map="auto",
            trust_remote_code=True  # Qwen3 可能需要信任远程代码
        )
...
class QwenLoRA(BaseEngine):
    def __init__(self, base_model_path: str, lora_path: str):
        # 不调用父类的 __init__，因为我们需要特殊处理
        self.name = "QwenLoRA"
        self.temperature = 0.2
        self.top_p = 0.9
        self.max_tokens = 1024
...
class Gemini(BaseEngine):
    def __init__(self, model_name_or_path: str, api_key: str = "", base_url="https://api.kwwai.top/v1"):
        self.name = "Gemini"
        self.model = model_name_or_path
        self.base_url = base_url
        self.temperature = 0.2
        ...
        if api_key != "":
            ...
            self.api_key = os.environ.get("API_KEY", "sk-WWR97t6XLFoX560C7CauIXXZaCUssOHbyMel1EXj2kqkwPCY")
        self.client = OpenAI(api_key=self.api_key, base_url=self.base_url)
